    ...
    try:
        ...
        for p in pairs:
            ...
            participants_data.append({
                ...
                "pin_initial": p['pin'], # Store Plaintext Initial PIN for Admin Convenience
...
def admin_reset_pin_db(p_id: str, new_initial_pin: str, new_encrypted_target: str):
    ...
    try:
        data = {
            "pin_initial": new_initial_pin, # Store Plaintext Initial PIN for Admin Convenience
...
def main():
    ...
    st.markdown("<div style='text-align:center;color:#AAA;font-size:12px;margin-top:50px;'>Amigo Secreto v1.1.1 • Secure Hash + Admin Pin View</div>", unsafe_allow_html=True)
        ...
        if not participants:
            ...
            for p in participants:
                with st.expander(f"👤 {p['name']}", expanded=False):
                    ...
                    if p['must_change_pin']:
                        pin_display = f"🔑 PIN Inicial: **{p.get('pin_initial', 'N/A')}**"
                    else:
                        pin_display = "🔒 PIN já trocado pelo participante (não exibido)"
                        ...
                        if admin_blob:
                            ...
                            if target:
                                ...
                                if admin_reset_pin_db(p['id'], new_initial, new_enc):
                                    ...
                                    time.sleep(5)
                                    st.rerun() # Refresh to show new initial pin in expander
                                else:
                                    st.error("Erro DB")
                            else:
