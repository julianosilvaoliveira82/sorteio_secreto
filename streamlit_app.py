...
def main():
    ...
    st.markdown("<div style='text-align:center;color:#AAA;font-size:12px;margin-top:50px;'>Amigo Secreto v1.0.0 • Secure Hash • Single Cycle</div>", unsafe_allow_html=True)
...
def view_admin():
    ...
    if not st.session_state.admin_logged:
        ...
        st.markdown("<div class='standard-card'><p>Digite o PIN de Administrador.</p></div>", unsafe_allow_html=True)
...
def view_participant(p_id):
    ...
    st.markdown("<div style='text-align:center;'><span style='background:#E8F5E9;color:#2E7D32;padding:5px 12px;border-radius:20px;font-weight:bold;'>🔒 AMBIENTE SEGURO</span></div>", unsafe_allow_html=True)
    ...
    if not st.session_state.user_auth:
        st.markdown("<div class='standard-card'><h3>Digite seu PIN</h3>", unsafe_allow_html=True)
    ...
    if p['must_change_pin']:
        st.markdown("<div class='standard-card'><h3>⚠️ Defina sua Senha</h3>", unsafe_allow_html=True)
        ...
        if st.button("SALVAR E ABRIR", type="primary"):
            # Hash check against initial to ensure difference
